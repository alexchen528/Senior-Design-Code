...
ax[0].step(x_step,
               np.array(y_step) * norm_to_obsenum,
               where = "mid",
               color = "red",
               label = r"Proton MC")
#Iron 
...
x_step = np.hstack([np.hstack([x[0]-(x[1]-x[0]), x]), x[-1]+(x[-1]-x[-2])])
y = [int(i) for i in iro_arr_lowen_up_arr[:, 1]]
y_step = np.hstack([np.hstack([0, y]), 0])
norm_to_obsenum = np.sum(yobs_up) / np.sum(y)
ax[0].step(x_step,
               np.array(y_step) * norm_to_obsenum,
...
yobs_low = [int(i) for i in sd_arr_lowen_low_arr[:, 1]]
ax[1].errorbar(x,
               yobs_low,
               yerr = np.sqrt(yobs_low),
               fmt = "o",
               color = "k",
               label = "Observation")
...
y_step = np.hstack([np.hstack([0, y]), 0])
norm_to_obsenum = np.sum(yobs_low) / np.sum(y)
ax[1].step(x_step,
               np.array(y_step) * norm_to_obsenum,
               where = "mid",
               color = "red",
...
x_step = np.hstack([np.hstack([x[0]-(x[1]-x[0]), x]), x[-1]+(x[-1]-x[-2])])
y = [int(i) for i in iro_arr_lowen_low_arr[:, 1]]
y_step = np.hstack([np.hstack([0, y]), 0])
norm_to_obsenum = np.sum(yobs_low) / np.sum(y)
ax[1].step(x_step,
               np.array(y_step) * norm_to_obsenum,
...
x_step = np.hstack([np.hstack([x[0]-(x[1]-x[0]), x]), x[-1]+(x[-1]-x[-2])])
y = [int(i) for i in iro_arr_miden_up_arr[:, 1]]
y_step = np.hstack([np.hstack([0, y]), 0])
norm_to_obsenum = np.sum(yobs_up) / np.sum(y)
ax[0].step(x_step,
               np.array(y_step) * norm_to_obsenum,
...
yobs_low = [int(i) for i in sd_arr_miden_low_arr[:, 1]]
ax[1].errorbar(x,
               yobs_low,
               yerr = np.sqrt(yobs_low),
               fmt = "o",
...
x_step = np.hstack([np.hstack([x[0]-(x[1]-x[0]), x]), x[-1]+(x[-1]-x[-2])])
y = [int(i) for i in iro_arr_miden_low_arr[:, 1]]
y_step = np.hstack([np.hstack([0, y]), 0])
norm_to_obsenum = np.sum(yobs_low) / np.sum(y)
ax[1].step(x_step,
               np.array(y_step) * norm_to_obsenum,
...
x_step = np.hstack([np.hstack([x[0]-(x[1]-x[0]), x]), x[-1]+(x[-1]-x[-2])])
y = [int(i) for i in iro_arr_highen_up_arr[:, 1]]
y_step = np.hstack([np.hstack([0, y]), 0])
norm_to_obsenum = np.sum(yobs_up) / np.sum(y)
ax[0].step(x_step,
               np.array(y_step) * norm_to_obsenum,
...
yobs_low = [int(i) for i in sd_arr_highen_low_arr[:, 1]]
ax[1].errorbar(x,
               yobs_low,
               yerr = np.sqrt(yobs_low),
               fmt = "o",
...
x_step = np.hstack([np.hstack([x[0]-(x[1]-x[0]), x]), x[-1]+(x[-1]-x[-2])])
y = [int(i) for i in iro_arr_highen_low_arr[:, 1]]
y_step = np.hstack([np.hstack([0, y]), 0])
norm_to_obsenum = np.sum(yobs_low) / np.sum(y)
ax[1].step(x_step,
               np.array(y_step) * norm_to_obsenum,
...
x_step = np.hstack([np.hstack([x[0]-(x[1]-x[0]), x]), x[-1]+(x[-1]-x[-2])])
y = [int(i) for i in iro_LD_lowen_up_prior[:, 1]]
y_step = np.hstack([np.hstack([0, y]), 0])
norm_to_obsenum = np.sum(yobs_up) / np.sum(y)
ax[0].step(x_step,
              np.array(y_step) * norm_to_obsenum,
...
yobs_low = [int(i) for i in sd_LD_lowen_low_prior[:, 1]]
ax[1].errorbar(x,
               yobs_low,
               yerr = np.sqrt(yobs_up),
               fmt = "o",
...
x_step = np.hstack([np.hstack([x[0]-(x[1]-x[0]), x]), x[-1]+(x[-1]-x[-2])])
y = [int(i) for i in iro_LD_lowen_low_prior[:, 1]]
y_step = np.hstack([np.hstack([0, y]), 0])
norm_to_obsenum = np.sum(yobs_low) / np.sum(y)
ax[1].step(x_step,
              np.array(y_step) * norm_to_obsenum,
...
x_step = np.hstack([np.hstack([x[0]-(x[1]-x[0]), x]), x[-1]+(x[-1]-x[-2])])
y = [int(i) for i in iro_LD_lowen_up_later[:, 1]]
y_step = np.hstack([np.hstack([0, y]), 0])
norm_to_obsenum = np.sum(yobs_up) / np.sum(y)
ax[0].step(x_step,
              np.array(y_step) * norm_to_obsenum,
...
yobs_low = [int(i) for i in sd_LD_lowen_low_later[:, 1]]
ax[1].errorbar(x,
               yobs_low,
               yerr = np.sqrt(yobs_up),
               fmt = "o",
...
x_step = np.hstack([np.hstack([x[0]-(x[1]-x[0]), x]), x[-1]+(x[-1]-x[-2])])
y = [int(i) for i in iro_LD_lowen_low_later[:, 1]]
y_step = np.hstack([np.hstack([0, y]), 0])
norm_to_obsenum = np.sum(yobs_low) / np.sum(y)
ax[1].step(x_step,
              np.array(y_step) * norm_to_obsenum,
...
x_step = np.hstack([np.hstack([x[0]-(x[1]-x[0]), x]), x[-1]+(x[-1]-x[-2])])
y = [int(i) for i in iro_LD_miden_up_prior[:, 1]]
y_step = np.hstack([np.hstack([0, y]), 0])
norm_to_obsenum = np.sum(yobs_up) / np.sum(y)
ax[0].step(x_step,
              np.array(y_step) * norm_to_obsenum,
...
yobs_low = [int(i) for i in sd_LD_miden_low_prior[:, 1]]
ax[1].errorbar(x,
               yobs_low,
               yerr = np.sqrt(yobs_up),
               fmt = "o",
...
x_step = np.hstack([np.hstack([x[0]-(x[1]-x[0]), x]), x[-1]+(x[-1]-x[-2])])
y = [int(i) for i in iro_LD_miden_low_prior[:, 1]]
y_step = np.hstack([np.hstack([0, y]), 0])
norm_to_obsenum = np.sum(yobs_low) / np.sum(y)
ax[1].step(x_step,
              np.array(y_step) * norm_to_obsenum,
...
x_step = np.hstack([np.hstack([x[0]-(x[1]-x[0]), x]), x[-1]+(x[-1]-x[-2])])
y = [int(i) for i in iro_LD_miden_up_later[:, 1]]
y_step = np.hstack([np.hstack([0, y]), 0])
norm_to_obsenum = np.sum(yobs_up) / np.sum(y)
ax[0].step(x_step,
              np.array(y_step) * norm_to_obsenum,
...
yobs_low = [int(i) for i in sd_LD_miden_low_later[:, 1]]
ax[1].errorbar(x,
               yobs_low,
               yerr = np.sqrt(yobs_up),
               fmt = "o",
...
x_step = np.hstack([np.hstack([x[0]-(x[1]-x[0]), x]), x[-1]+(x[-1]-x[-2])])
y = [int(i) for i in iro_LD_miden_low_later[:, 1]]
y_step = np.hstack([np.hstack([0, y]), 0])
norm_to_obsenum = np.sum(yobs_low) / np.sum(y)
ax[1].step(x_step,
              np.array(y_step) * norm_to_obsenum,
...
x_step = np.hstack([np.hstack([x[0]-(x[1]-x[0]), x]), x[-1]+(x[-1]-x[-2])])
y = [int(i) for i in iro_LD_highen_up_prior[:, 1]]
y_step = np.hstack([np.hstack([0, y]), 0])
norm_to_obsenum = np.sum(yobs_up) / np.sum(y)
ax[0].step(x_step,
              np.array(y_step) * norm_to_obsenum,
...
yobs_low = [int(i) for i in sd_LD_highen_low_prior[:, 1]]
ax[1].errorbar(x,
               yobs_low,
               yerr = np.sqrt(yobs_up),
               fmt = "o",
...
x_step = np.hstack([np.hstack([x[0]-(x[1]-x[0]), x]), x[-1]+(x[-1]-x[-2])])
y = [int(i) for i in iro_LD_highen_low_prior[:, 1]]
y_step = np.hstack([np.hstack([0, y]), 0])
norm_to_obsenum = np.sum(yobs_low) / np.sum(y)
ax[1].step(x_step,
              np.array(y_step) * norm_to_obsenum,
...
x_step = np.hstack([np.hstack([x[0]-(x[1]-x[0]), x]), x[-1]+(x[-1]-x[-2])])
y = [int(i) for i in iro_LD_highen_up_later[:, 1]]
y_step = np.hstack([np.hstack([0, y]), 0])
norm_to_obsenum = np.sum(yobs_up) / np.sum(y)
ax[0].step(x_step,
              np.array(y_step) * norm_to_obsenum,
...
yobs_low = [int(i) for i in sd_LD_highen_low_later[:, 1]]
ax[1].errorbar(x,
               yobs_low,
               yerr = np.sqrt(yobs_up),
               fmt = "o",
...
x_step = np.hstack([np.hstack([x[0]-(x[1]-x[0]), x]), x[-1]+(x[-1]-x[-2])])
y = [int(i) for i in iro_LD_highen_low_later[:, 1]]
y_step = np.hstack([np.hstack([0, y]), 0])
norm_to_obsenum = np.sum(yobs_low) / np.sum(y)
ax[1].step(x_step,
              np.array(y_step) * norm_to_obsenum,
